...
Today we will perform the RNAseq analysis on the effects of
dexamethasome (hereafter “dex”), a common steroid, on airway smooth
...
We have four replicates per condition (“control” and “treated”). We want
...
lead to “funky” log2 fold change values (i.e. divide by zero errors
etc.)
...
We will use the **gage** package and the **pathview** package to do this
...
To run our pathway analysis, we will use the `gage()` function. It wants
two main inputs: a vector of importance (in our case, the log2 fold
change values) and the gene sets to check overlap with.
